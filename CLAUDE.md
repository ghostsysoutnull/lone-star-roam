...
See `ROADMAP.md` for planned work and known limitations before proposing features. Active track: **Railroads Realism** (`RAILS_SPEC.md`, 3 waves, first track of the 2026-H2 program — `VISION_2026H2.md`). Shipped tracks: aviation 2026-07-12, Agriculture (`AGRICULTURE_SPEC.md`, all 5 waves + 4.5/5.5 follow-ons) 2026-07-14, the Shoulder & the Shelf 2026-07-15, Band Parity (`BAND_PARITY_SPEC.md`, 6 waves) 2026-07-17, New Player Experience (`NEWPLAYER_SPEC.md`, 4 waves) 2026-07-17, and Energy (`ENERGY_SPEC.md`, 6 waves + the W4.5 solar rework) 2026-07-18 — all folded into `ROADMAP.md`. Queued work lives in `BACKLOG.md`. **`GOTCHAS.md` is the law book** — standing rules that survived their tracks; grep it for the area you're changing before coding there. `LEDGER.md` is the per-wave scoreboard (one line per wave, appended at wave end).
...
1. **Spec session**: write `<NAME>_SPEC.md` — goals, the wave split, and open calls resolved *before* any wave codes. Each wave = one session's deliverable: the code + its verify checks. **Spec format**: the file opens with an executive summary — **Goal** line, then per wave a "**Wave N — the player gets:**" player-facing bullet list (one item per line) closed by a flat dev-register *Expected result:* paragraph (short verifiable statements, no narrative) and a *Suggested setup:* line (model + effort for the wave — Bruno sets these manually via `/model` and `/effort`). Technical wave sections below keep their lists one-item-per-line too. `NEWPLAYER_SPEC.md` is the reference example.
2. **Per wave, recommend model + effort** — grade by **where the risk lives**, not what most keystrokes are: a wave carrying a new visible surface, new-system architecture, or feel-tuning gets **Fable 5** even if it's 80% plumbing (the Energy-W3 solar lesson: a "structural" grade shipped a look that needed the Fable W4.5 rework); pure execution of settled design (bakes, table extensions, suite plumbing) gets **Sonnet 5**. Specs and tech design are always Fable 5, and each spec ships per-wave **"design settled"** sections (contracts, module touchpoints, resolved calls) so Sonnet waves execute decisions rather than make them. Two-model rotation only — no third model; it keeps the ledger's model-vs-ROI comparison and the game's register clean. Effort usually high. Also state the wave's token budget (e.g. "code + checks, no shots, grep-first") and its perf cost (expected draw-call/triangle delta against the caps in `tools/checks/perf.mjs`, or "none" for logic-only waves).
