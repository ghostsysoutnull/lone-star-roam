...
- `shop.js` — 🛒 Shop tab catalog + purchases. `applyGear()` turns `save.gear` levels into `player.perks`; vehicle.js reads `perks` (never the save) in its DRIVE branch. Index 0 of each knob array is the stock value and **must match vehicle.js/gameplay defaults**. Prices/effects all live at the top of this file.
