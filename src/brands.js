...
export class BrandSystem {
  constructor(scene, { onHum } = {}) {
    ...
    // buildingMat) — the glow material's emissiveIntensity is the single knob
    // toggled by ATMOS.night in update().
  ...
  spawn(site) {
    ...
    // Hero: storefront + sign pylon + beaver. Split into non-glow (heroMat)
    // and glow (glowMat: sign face, beaver, canopy soffit) merged meshes.
    ...
    this.live.set(site.name, { group, glowMesh, staticMesh, pumps, boards });
  }
...
// Bucky's hero — long storefront + red roofline + glass front, a wide fuel
// canopy (glowing soffit) over N pump islands, and a tall roadside sign pylon
// topped by the beaver. Returns two merged geometries (non-glow / glow: sign
// face, beaver, canopy soffit) plus the pump-island transforms. Built in the
// group's LOCAL frame: origin at pad center, ground y = 0, +z = toward the
// road (the group is rotated so its front faces the highway).
function buildBuckyHero(skirt = 0.4) {
  const s = [], glow = []; // static / glow part arrays
  ...
  s.push(tinted(new THREE.BoxGeometry(27.2, 0.8, 0.6).translate(0, 4.9, -1.3), TRIM));    // red roofline band (front)
  s.push(tinted(new THREE.BoxGeometry(26, 3.0, 0.3).translate(0, 1.9, -1.35), GLASS));    // glass storefront strip
  // entry parapet — the raised centerpiece
  s.push(tinted(new THREE.BoxGeometry(7, 2.2, 1.4).translate(0, 6.0, -1.6), WALL));
  s.push(tinted(new THREE.BoxGeometry(7.4, 0.5, 1.6).translate(0, 7.2, -1.6), TRIM));
  ...
  // --- fuel canopy (toward the road), soffit glows white at night ---
  ...
  s.push(tinted(new THREE.BoxGeometry(CW + 0.6, 0.5, CD + 0.6).translate(0, 6.7, CZ), TRIM)); // red fascia band
  glow.push(tinted(new THREE.BoxGeometry(CW - 0.6, 0.12, CD - 0.6).translate(0, 5.92, CZ), CANOPY)); // glowing soffit
  ...
  glow.push(tinted(new THREE.BoxGeometry(5.6, 4.2, 0.5).translate(SX, 14.6, SZ + 0.65), SIGN)); // glowing yellow sign face
  glow.push(tinted(new THREE.BoxGeometry(5.6, 0.6, 0.55).translate(SX, 12.6, SZ + 0.66), TRIM)); // sign underline (glows)
  for (const p of beaverParts(SX, 18.4, SZ + 0.2)) glow.push(p);

  return { staticGeo: merge(s), glowGeo: merge(glow), pumpXforms };
}
