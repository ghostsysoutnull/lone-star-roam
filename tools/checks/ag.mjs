...
export default async function ag(t) {
  ...
  await t.check('new species are registered with facts (log-ready, additive)', async () => {
    ...
    t.ok(res.count === 34, `SPECIES_COUNT drifted: ${res.count}`); // 20 + the wave-5 trio + wave-5b hereford + W3 Kemp's ridley + W4 dolphin + W5 spoonbill/crane + W6a black bear + Sea W2's five offshore
  });
