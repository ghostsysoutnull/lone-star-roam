...
export default async function band(t) {
  ...
  // 169/177 after LA + NM joined the secondary tier too (AR skipped both
  // times — already 15/15). 8 places remain uncovered, all small OK hamlets
  // plus Logan, NM — genuinely off even the secondary network. Asserted as a
  // floor, not exact, so a legitimate future rebake that improves coverage
  // still passes — only a regression hides silently otherwise.
