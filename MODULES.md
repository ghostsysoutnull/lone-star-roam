...
- `radio.js` — `TowerRadio`: `receivable` (FLY-in-range or `perks.avionics`), `update(dt, player, aviation, sky)`, `atis`/`narrateOps`/`playerFlow`/`checkBlock` (physical go-around, reception-independent), `tx`/`lastTx` (structured, for tests), `TOWERED`/`TOWERED_COUNT` (the 7 tier-1 hubs).
