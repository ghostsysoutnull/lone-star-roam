...
- `airports.js` — `AIRPORTS` table (true headings from OSM; `RW_W`/`MARGIN` tier knobs), `airportClear` (pure footprint exclusion — called by cities.js/ScenerySystem/`chapelAt`), `airportLayout` (pure, pads at max `hAt`), `windFrom` (per-day seeded wind direction — waves 2/3 must reuse), `AirportSystem` (8 merged meshes in `this.group`; beacon spin + windsock droop in `update`).
