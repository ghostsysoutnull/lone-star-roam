...
- `hud.js` — `HUD`: `update` (12 Hz; location/road/speed/mode/forecast lines), `renderMapLayer` (one offscreen canvas), `drawMini`/`drawBig`/`drawCompass`, `uiScale` (rem root), `toast`/`dialog`/`interactHint`/`subtitle` (tower radio, one-line queue), `toggleBigMap`/`toggleHelp`/`cycleZoom`.
- `audio.js` — `AudioSystem`: `update` (engine/wind/rain/crickets/theremin), `chime` (`SONGS` table), `radio(text, opts)` (tower-radio synth; `radioDuck` gain node, `opts.ufo` chop), `honk`/`bark`/`howl`/`rattle`/`gobble`/`trainHorn`/`thunder`/`flare`/`step`, `toggleMute`.
...
- `radio.js` — `TowerRadio`: `receivable` → `{a, kind: 'tower'|'unicom'}` (FLY-in-range — 250u tower/120u UNICOM — or `perks.avionics`), `update(dt, player, aviation, sky)`, `atis`/`awos`/`narrateOps`/`playerFlow` (tower only)/`checkBlock` (physical go-around, reception-independent, all 20 fields), `tx`/`lastTx` (structured, for tests), `TOWERED`/`TOWERED_COUNT` (7 tier-1 hubs), `UNICOM` (9 tier-2 fields), `nearestTowered`/`nearestUnicom`.
