...
- `world.js` — `buildWorld` → `buildTerrain`/`buildHighways`/`buildWater`/`buildCountyLines`/`buildRibbons` (all roads = merged meshes per tier); `ScenerySystem` inside `buildWorld` return (chunked flora/props, `group.userData.animated`).
...
- `gameplay.js` — `Gameplay`: save load/`persist` (keys: cities/landmarks/roses/species/stats/counties/ufo/bank/jobsDone/job/gear), `LANDMARKS` (+`LL` projection), `enterCounty`, `spotSpecies`, `ufoSighting`, `landmarkNear`, `mkRoses` (rose RNG — never touch).
...
- `checks/*.mjs` — suites: drive/hud/lights/missions/shop/traffic/wildlife (one default-export async fn each).
